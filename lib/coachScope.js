 ...
 *
 * IL DISAIT LEVER UNE LIMITE QUI N'EXISTE PLUS
 *
 * SCOPE_ENABLED vaut false : Nuvi ne refuse rien a personne. La bascule
 * proprietaire, elle, ne consulte pas ce drapeau, donc la passphrase
 * repondait encore "le perimetre est leve, je reponds a tout" - une promesse
 * de liberte sur une prison vide, et pire, l'affirmation qu'il existait une
 * prison. Quand le perimetre est desactive, le message le dit.
 *
 * ET IL PORTAIT UN CADRATIN
 *
 * Ecrit "\u2014", donc invisible a la lecture du fichier comme au balayage
 * qui refusait les cadratins - celui-ci ne lisait que le caractere. Il
 * s'affichait pourtant bel et bien dans la conversation. tests/no-em-dash
 * lit maintenant les deux formes.
 */
export function scopeUnlockNotice(unlocked, lang = "fr") {
  if (!SCOPE_ENABLED) {
    return lang === "en"
      ? "Owner mode noted. There is nothing to lift: Nuvi answers everything, for everyone."
      : "Mode proprietaire note. Il n'y a rien a lever : Nuvi repond a tout, pour tout le monde.";
  }
  if (lang === "en") {
    return unlocked
