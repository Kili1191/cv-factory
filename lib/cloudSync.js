...
export const SYNCED_KEYS = [
  ...
  "cvf_rep", // les reponses qu'on repete a chaque candidature
  // LE MARCHE SUIT LA PERSONNE, PAS L'APPAREIL
  //
  // Il decide l'orthographe britannique, le format des dates et ce qu'un
  // CV porte dans ce pays : presque tout lib/conventions.js. Il est reste
  // hors de cette liste, donc un choix fait sur l'ordinateur n'arrivait
  // pas sur le telephone. AppRoot le devine alors depuis le lieu ecrit sur
  // le CV, ce qui le retrouve quand le CV dit Londres et se trompe pour
  // quelqu'un qui habite Paris et postule a Londres. Rien a l'ecran ne
  // disait que le reglage n'avait pas suivi : le CV partait simplement
  // dans les conventions d'un autre pays.
  "cvf_pays",
];
...
export function initCloud(onRemoteChange) {
  ...
  // The client arrives asynchronously now (the library loads on demand).
  // The caller keeps a synchronous unsubscribe: it flips a flag so a
  // client that arrives after the effect was torn down is left unused.
  let arrete = false;
  let sub = null;
  ...
  getSupabase().then((sb) => {
    ...
    sub = sb.auth.onAuthStateChange((_event, session) => { handle(session); }).data;
  }).catch(() => emit({ status: "off" }));

  return () => {
    arrete = true;
    try { if (sub) sub.subscription.unsubscribe(); } catch { /* deja parti */ }
  };
}
