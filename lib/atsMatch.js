...
// UNE EXIGENCE NE COMMENCE PAS PAR "ACROSS" ET NE FINIT PAS PAR "ACROSS"
//
// Trouve en se mettant a la place du client, sur une annonce de Beverage
// Manager. Le panneau donnait a ajouter au CV : "across two", "across both",
// "both bars", "experience running", "management across", "essential level",
// "experienced beverage". Ce ne sont pas des exigences, ce sont des morceaux
// de phrase decoupes au hasard des suites de deux mots. Le depot dit deja ce
// que ca coute : le jour ou le panneau propose trois lignes qui ne veulent
// rien dire, on cesse de lire les vingt autres, y compris les bonnes. Et
// elles comptaient au denominateur du chiffre, ce qui l'ecrasait.
//
// Une expression qui porte du sens ne commence ni ne finit par un mot de
// liaison, un determinant, un nombre ecrit, ou un mot qui qualifie l'exigence
// au lieu de la nommer ("essential", "desirable", "experienced").
const FRONTIERE = new Set(
  ("across both from into onto within without during between including alongside "
   + "toward towards under over about through per plus versus via and or but "
   + "essential desirable preferred required mandatory ideally advantageous "
   + "experience experienced experiencing running working having being "
   + "one two three four five six seven eight nine ten twelve fifteen twenty "
   + "several many various multiple other another such same own "
   + "deux trois quatre cinq dix quinze vingt plusieurs autre autres meme propre "
   + "souhaite souhaitee exige exigee requis requise indispensable apprecie").split(" ")
);

function bordsSolides(p) {
  const mots = p.split(" ");
  if (mots.length < 2) return !FRONTIERE.has(mots[0]);
  return !FRONTIERE.has(mots[0]) && !FRONTIERE.has(mots[mots.length - 1]);
}
...
const MOBILIER = [
  ...
  // RELEVE SUR UNE ANNONCE DU CIVIL SERVICE, LE 25 SEPTEMBRE 2026
  //
  // Le panneau donnait a ajouter au CV : "new window", "opens",
  // "candidates". Les deux premiers viennent du texte d'un lien,
  // "(opens in a new window)", que rien ne distinguait d'une exigence ;
  // le troisieme est le mot par lequel une annonce designe la personne qui
  // la lit. "opens" n'est refuse que seul ou devant le lien : une annonce
  // qui demande d'ouvrir des comptes a le droit de le dire.
  /\bnew (window|tab)\b/,
  /^opens?$/,
  /\bopens in\b/,
  // Un jour de la semaine ou un mois : c'est une date limite, jamais une
  // competence. Trouve par le test, qui laissait passer "sunday opens",
  // pris dans "Apply before 11:55 pm on Sunday (opens in a new window)".
  /\b(monday|tuesday|wednesday|thursday|friday|saturday|sunday|lundi|mardi|mercredi|jeudi|vendredi|samedi|dimanche)\b/,
  /\b(january|february|march|april|june|july|august|september|october|november|december)\b/,
  /\b(candidates?|applicants?|vacanc(y|ies)|recruitment|shortlist(ed|ing)?)\b/,
  // Les intitules du formulaire d'une annonce publique : la grille, la
  // pension, le service employeur. Ce sont des rubriques de la page, et la
  // personne n'en met aucune sur son CV.
  /\b(pension|employer contribution|job grade|business area|type of role|working pattern|number of jobs available|reserved for|nationality requirements)\b/,
];
