...
export function defautsVisuels(doc, locale = "fr") {
  ...
  // L'ECRAN EST AGRANDI, LA FEUILLE NE L'EST PAS
  //
  // Sur ordinateur l'apercu grossit le document jusqu'a 1,35 par un
  // transform, et getBoundingClientRect rend les rectangles APRES ce
  // grossissement. Un CV de 1069px se mesurait donc a 1443px : "plus haut
  // qu'une page", et son titre EDUCATION, a 1000px reels, tombait "en bas
  // de page" a 1350px transformes. Le controle accusait un CV qui tenait.
  // offsetHeight ignore le transform : le rapport des deux donne l'echelle,
  // et toute position verticale est ramenee a la feuille reelle.
  const echelle = el.offsetHeight ? cadre.height / el.offsetHeight : 1;
  const reel = (px) => px / (echelle || 1);
  ...
  if (reel(cadre.height) > HAUTEUR_PAGE) {
    ...
    for (const n of titres) {
      const y = reel(n.getBoundingClientRect().top - cadre.top);
