...
const TXT = {
  fr: {
    ...
    langue_sans_nom: "un niveau sans langue : la ligne s'affichera sans dire "
      + "de quoi elle parle",
    coupe_en_fin: "cette ligne s'arrete au milieu d'une phrase : le texte a ete "
      + "coupe en le copiant, et la suite manque",
  ...
  en: {
    ...
    coupe_en_fin: "this line stops mid-sentence: the text was cut when it was "
      + "copied, and the rest is missing",
...
export function defautsDuCv(cv, locale = "fr") {
  ...
  // 6 bis. UNE LIGNE QUI S'ARRETE AU MILIEU D'UNE PHRASE
  //
  // Le texte copie depuis un PDF dont la couche etait amputee arrivait
  // avec "Europe an", "by 30% by", "a structured che" en fin de puce. Le
  // lecteur les a rangees telles quelles, et rien ne le disait : un CV
  // avec des phrases coupees partait chez un recruteur. Une ligne qui
  // finit sans ponctuation sur un mot-outil ("an", "by", "the", "de",
  // "et") n'est pas une phrase que quelqu'un a ecrite ainsi.
  ...
  const coupeEnFin = (s) => {
    const t = texte(s);
    return t.length >= 30 && !/[.!?:;)"\u00bb%\d]$/.test(t) && MOT_OUTIL.test(t);
  };
  exp.forEach((e, i) => {
    (Array.isArray(e.bullets) ? e.bullets : []).forEach((b) => {
      if (coupeEnFin(b)) out.push(defaut("coupe_en_fin", L.ou.experience(i + 1), b, L.coupe_en_fin));
    });
  });
  if (coupeEnFin(cv.summary)) out.push(defaut("coupe_en_fin", L.ou.accroche, cv.summary, L.coupe_en_fin));
...
export function defautsVisuels(doc, locale = "fr") {
  ...
  // 2. IL N'Y A PLUS DE "TITRE SEUL EN BAS DE PAGE"
  //
  // Une regle vivait ici : un titre de section a moins de 60px du bas
  // d'une page, avec sa section sur la suivante. Elle datait du PDF a deux
  // feuilles. Le PDF n'a plus qu'une feuille : le document y tient tel
  // quel, ou reduit, ou ne part pas. Aucune coupure de page n'existe plus,
  // donc aucun titre ne peut en etre orphelin. Elle a pourtant accuse
  // LANGUAGES sur le CV de Kilian, un CV qui partait entier sur une page :
  // une alerte sur un defaut impossible, sur le seul geste qui ne se
  // rattrape pas. Elle est retiree, et la cle "titre_orphelin" avec elle.
...
const ORDRE = ["sans_nom", "sans_contact", "recouvrement", "deborde",
  ...
  "poste_anonyme", "poste_muet", "doublon", "coupe_en_fin", "langue_sans_nom"];
