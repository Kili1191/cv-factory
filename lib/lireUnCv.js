...
const SECTIONS = {
  ...
  skills: /^(competences?|skills|expertise|savoir-faire|technical skills|top skills)\b/i,
  languages: /^(langues?|languages?)\b/i,
  certifications: /^(certifications?|certificats?|licences?|accreditations?)\b/i,
  // A LINKEDIN PROFILE SAVED AS PDF HAS HEADINGS OF ITS OWN
  //
  // "Contact" over the e-mail and the profile link, "Top Skills" over
  // three skills, "Honors-Awards", "Publications". Read as text they used
  // to land in whatever section came before them: "Contact" became a
  // degree. Contact lines are found from the whole text anyway, and the
  // sections the CV has no place for are set aside rather than misfiled.
  contact: /^(contact|coordonnees)\b/i,
  autres: /^(honors?|awards?|distinctions?|publications?|interests?|centres? d'interet|loisirs|hobbies|volunteer|benevolat|recommendations?|activities|patents?|courses|projects?|projets?)\b/i,
};
...
// "Page 1 of 2" at the foot of every LinkedIn page, and "3 years 8 months"
// under an employer with several roles: furniture of the export, not of
// the career. They would become a bullet and an employer.
const BRUIT_DE_PAGE = /^(page \d+ (of|sur|\/) \d+|\(?\d+\s+(years?|months?|ans?|mois)(\s+\d+\s+(months?|mois))?\)?)$/i;

// A SENTENCE WRAPPED BY THE PAGE IS ONE SENTENCE
//
// Text read out of a PDF comes in physical lines: a description that the
// page wrapped arrives as "...revenu annuel recurrent" then "sur deux
// continents." The second half, alone, was a sentence and the first was
// not, so the first became the heading of a new job called "sur deux
// continents.". A long line with no closing punctuation, followed by a
// line that starts in lower case, is one line the page cut; headings and
// job titles are short and start with a capital, so they are never joined.
function recoller(ls) {
  const out = [];
  for (const l of ls) {
    const prev = out[out.length - 1];
    if (prev && prev.length > 40 && !/[.!?:;]$/.test(prev) && /^[a-z\u00e0-\u00ff]/.test(l)) {
...
function lignes(texte) {
  return recoller(String(texte || "")
    .replace(/\r/g, "\n")
    .split("\n")
    .map((l) => l.replace(/\u00a0/g, " ").replace(/[ \t]+/g, " ").trim())
    .filter((l) => l.length > 0 && !BRUIT_DE_PAGE.test(l)));
}
...
function sansLaPeriode(t) {
  return String(t || "")
    .replace(PERIODE, "")
    // LinkedIn writes the length after the period: "January 2021 -
    // Present (3 years 8 months)". Without this the bracket became a
    // location.
    .replace(/\(\s*(?:\d+\s+(?:years?|months?|ans?|mois)\s*)+\)/gi, " ")
...
// TWO LANGUAGES ON ONE LINE ARE TWO LANGUAGES
//
// Templates draw the languages on one row, so a CV read back from its own
// PDF, or from a photo of it, hands the reader "French: Native English:
// Professional fluency" as a single piece: no bullet, no comma, nothing
// decouper splits on. It became one language, "French", with the level
// "Native English: Professional fluency", printed as such on the next
// download. A second "Word:" inside the piece is the seam; the same for
// a second "(level)". A level that is itself two words ("Professional
// fluency") is safe: the seam needs the colon or the closing bracket.
function couplesDeLangues(x) {
  const t = String(x || "").trim();
  const colons = (t.match(/:/g) || []).length;
  if (colons >= 2) {
    // One word before the colon, not two: "Native English:" would cut at
    // "Native". A two-word language name after the first couple ("Swiss
    // German") is the rarer shape and loses; the level after "French:" is
    // the common one.
    return t.split(/\s+(?=[A-Z\u00c0-\u00dd][\w\u00e0-\u00ff-]*\s*:)/)
      .map((p) => p.trim()).filter(Boolean);
  }
  if ((t.match(/\)/g) || []).length >= 2) {
    return t.split(/(?<=\))\s+(?=[A-Z\u00c0-\u00dd])/).map((p) => p.trim()).filter(Boolean);
  }
  return [t];
}
...
export function lireUnCv(texte) {
  ...
  const education = blocs(sec.education || []).map((b, i) => {
    ...
    // LinkedIn puts the school above the degree; most CVs do the reverse.
    // A first line that names a school and a second that does not is the
    // LinkedIn order.
    const ecole = (x) => /\b(school|university|universite|ecole|college|institut|institute|academy|academie|lycee|faculte|faculty|polytechnique|iut|cfa)\b/i.test(x || "");
    const [degree, school] = t.length >= 2 && ecole(t[0]) && !ecole(t[1]) ? [t[1], t[0]] : [t[0], t[1]];
    return { id: i + 1, degree: degree || "", school: school || "", period: per };
  ...
  const cv = {
    ...
    languages: listeDe(sec.languages || []).flatMap(couplesDeLangues).map((x) => {
