...
function controles(cv, parsed, texte) {
  ...
  // CE CONTROLE A ETE REECRIT PARCE QU'IL SE TROMPAIT
  //
  // Il cherchait les intitules de rubrique inventes en reperant les lignes
  // courtes tout en majuscules. Sur un vrai PDF exporte, il a signale "SQL"
  // comme rubrique inventee : c'est une competence, seule sur sa ligne, en
  // capitales. Le CV etait lu a 100% par les vrais moteurs, et la simulation
  // le recalait chez quatre analyseurs sur six.
  //
  // Le defaut n'etait pas un reglage trop strict, c'etait le signal choisi.
  // "Court et en majuscules" ne distingue pas un titre de rubrique d'un
  // acronyme, et le public de Nuvi en porte partout : SIA, HACCP, EPOS, WSET.
  //
  // On mesure donc directement ce qui compte, sans deviner : la personne a des
  // postes, et l'analyseur a-t-il su reconnaitre la rubrique qui les contient ?
  // C'est exactement le mode d'echec redoute - une rubrique intitulee "Mon
  // parcours" n'est jamais reconnue et le bloc entier disparait - et il se lit
  // dans la sortie de l'analyseur au lieu d'etre infere du texte brut.
  const rubriquesVues = (parsed.sectionsFound || []);
  const experiencePerdue = postes.length > 0 && !rubriquesVues.includes("experience");
  const formationPerdue = ((cv && cv.education) || []).filter(Boolean).length > 0
    && !rubriquesVues.includes("education");
  ...
  return {
    ...
    rubriques: {
      ok: rubriquesVues.length >= 2 && !experiencePerdue && !formationPerdue,
      fait: rubriquesVues.length + " rubrique(s) reconnue(s) : "
        + (rubriquesVues.join(", ") || "aucune")
        + (experiencePerdue
          ? " ; les postes existent mais la rubrique experience n'est pas reconnue" : "")
        + (formationPerdue
          ? " ; la formation existe mais sa rubrique n'est pas reconnue" : ""),
    },
    dates: {
