...
// THE ACCOUNT TRAVELS WITH THE CALL
//
// With billing on, the route needs to know who is asking: a subscriber,
// an account on its free fits, or nobody. The Supabase session's access
// token is the proof. Without a session the header is simply absent, and
// the route answers 401 when billing is on, nothing otherwise.
export async function jetonDuCompte() {
  try {
...
// 401 sign_in_required and 402 plan_required are not failures of the model:
// they are the product asking for an account or the plan. They are not
// retried, not reported as incidents, and the app hears them as one event.
const REPONSES_DU_PLAN = new Set(["sign_in_required", "plan_required"]);
function signalerLePlan(detail) {
  if (typeof window === "undefined") return;
  try {
    window.dispatchEvent(new CustomEvent("nuvi:paywall", { detail }));
  } catch { /* sans CustomEvent : l'erreur typee suffit */ }
}
...
export async function aiCall(prompt, options = {}) {
  ...
  for (let tentative = 1; tentative <= AI_TENTATIVES; tentative++) {
    ...
    if (!r.ok || (d && d.error)) {
      ...
      const genre = d && d.error && d.error.type;
      if (REPONSES_DU_PLAN.has(genre)) {
        derniere.code = genre;
        derniere.plan = d.error;
        signalerLePlan({ type: genre, ...d.error, task_name });
        throw derniere;
      }
