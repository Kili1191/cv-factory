...
// THE LIBRARY LOADS WHEN A SESSION IS POSSIBLE, NOT BEFORE
//
// A static import here put 154 KB of Supabase into the first load of
// every page, accounts configured or not: the module is imported by the
// root for one synchronous question, isCloudConfigured(). That question
// is an environment check and stays synchronous; the client itself is
// created behind a dynamic import, so the library is fetched only on a
// deployment where accounts exist, and only once the app asks for it.
let cached;
