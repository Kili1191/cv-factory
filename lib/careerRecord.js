...
export function apportDuDossier(cvCourant, versions = []) {
  ...
  const expEnPlus = dossier.experience.filter(e => !clefsCourantes.has(clefExperience(e))).length;

  const skillsCourants = new Set((cur.skills || []).map(fold));
  const skillsEnPlus = dossier.skills.filter(s => !skillsCourants.has(fold(s))).length;
