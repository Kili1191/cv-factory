...
export function cleanupCv(cv, opts = {}) {
  const lang = opts.lang || "fr";
  if (!cv || typeof cv !== "object") {
    return { newCv: cv, changed: false, dedupRemoved: 0 };
  }
  ...
  const summary = dedupRemoved > 0
    ? (lang === "en"
      ? dedupRemoved + " duplicate(s) removed"
      : dedupRemoved + " doublon" + (dedupRemoved > 1 ? "s" : "") + " supprime" + (dedupRemoved > 1 ? "s" : ""))
    : "";
