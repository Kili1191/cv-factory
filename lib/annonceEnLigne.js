...
export function pageDepuisLeHtml(html, stripTags) {
  ...
  // The heading, for the pages whose title names the kind of page rather
  // than the job. Same field the extension collects in the browser.
  const h1 = /<h1[^>]*>([\s\S]*?)<\/h1>/i.exec(texte);
  const corps = /<body[^>]*>([\s\S]*)<\/body>/i.exec(texte);
  const bodyText = texteDeLAnnonce(corps ? corps[1] : texte, stripTags);
  return { jsonLd, meta, bodyText, h1: h1 ? stripTags(h1[1]) : "" };
}
