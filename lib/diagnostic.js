...
import { compterLesPuces } from "./resultatOuResponsabilite.js";
...
// Une puce citee dans un conseil doit rester lisible d'un coup d'oeil : on la
// coupe a la fin d'un mot, jamais au milieu.
function court(t, max = 46) {
...
const AXES = [
  ...
  {
    // CE QU'ON VOUS A CONFIE, OU CE QUE VOUS AVEZ OBTENU
    //
    // L'axe "bullets" compte les chiffres et les debuts de puce en forme de
    // fiche de poste. Les deux sont utiles et aucun ne suffit : "Encadre une
    // equipe de 12 sur quatre points de vente" porte un chiffre, ne commence
    // pas par une formule plate, et ne dit toujours aucun resultat. 12 et 4
    // mesurent le perimetre confie, pas ce qui en est advenu.
    //
    // C'est pourtant la difference qui decide d'un rappel. Une
    // responsabilite se lit sur une fiche de poste : n'importe qui ayant tenu
    // ce poste pourrait l'ecrire. Un resultat ne se verifie qu'aupres de
    // vous.
    //
    // Le poids est eleve, juste sous celui des puces elles-memes : c'est le
    // seul axe qui separe deux CV au meme parcours.
    id: "achievements",
    poids: 1.5,
    mesure(cv) {
      const c = compterLesPuces(toutesLesPuces(cv));
      if (!c.total) return { note: 0, fait: { total: 0, resultat: 0, indetermine: 0 } };
      // Un resultat vaut son plein ; un resultat annonce sans chiffre vaut la
      // moitie, parce que l'intention est bonne et qu'il ne manque qu'une
      // mesure ; une responsabilite ne vaut rien pour cet axe, ce qui ne veut
      // pas dire qu'elle n'a rien a faire sur le CV.
      const part = (c.resultat + c.indetermine * 0.5) / c.total;
      return {
        note: borne(Math.round(part * 100)),
        fait: {
          total: c.total,
          resultat: c.resultat,
          indetermine: c.indetermine,
          responsabilite: c.responsabilite,
          exemples: c.exemples,
        },
      };
    },
...
const PHRASES = {
  fr: {
    ...
    // UN CONSEIL QUI NE CITE PAS LA PHRASE NE SE CORRIGE PAS
    //
    // "Montre des resultats" se relit et s'oublie : personne ne sait laquelle
    // de ses puces est visee ni ce qu'il faudrait y ajouter. On cite donc la
    // puce, on dit ce qu'elle raconte aujourd'hui, et on nomme ce qui manque.
    achievements: (f) => {
      if (!f.total) return "Ajoute des puces a tes experiences : un poste sans puce ne prouve rien.";
      const ex = f.exemples || {};
      if (f.responsabilite && ex.responsabilite) {
        return "\u00ab\u00a0" + court(ex.responsabilite.texte) + "\u00a0\u00bb dit ce qu'on t'a confie, "
          + "pas ce qui en est sorti. N'importe qui ayant tenu ce poste pourrait "
          + "l'ecrire. Ajoute le resultat : ce qui a change parce que tu etais la, "
          + "et de combien. " + f.responsabilite + " puce(s) sur " + f.total
          + " sont dans ce cas.";
      }
      if (f.indetermine && ex.indetermine) {
        return "\u00ab\u00a0" + court(ex.indetermine.texte) + "\u00a0\u00bb annonce un resultat sans le "
          + "mesurer. Un recruteur ne peut pas le distinguer d'un souhait. Mets le "
          + "chiffre : combien, de quoi a quoi, en combien de temps.";
      }
      return "Chaque puce dit un resultat. Plus rien a corriger a la mesure sur cet axe.";
    },
    bullets: (f) => f.total === 0
  ...
  en: {
    ...
    achievements: (f) => {
      if (!f.total) return "Add bullets to your jobs: a job with no bullets proves nothing.";
      const ex = f.exemples || {};
      if (f.responsabilite && ex.responsabilite) {
        return "\u201c" + court(ex.responsabilite.texte) + "\u201d says what you were given, "
          + "not what came of it. Anyone who held that job could write it. Add the "
          + "outcome: what was different because you were there, and by how much. "
          + f.responsabilite + " of " + f.total + " bullets read that way.";
      }
      if (f.indetermine && ex.indetermine) {
        return "\u201c" + court(ex.indetermine.texte) + "\u201d claims a result without "
          + "measuring it. A recruiter cannot tell it apart from a wish. Put the "
          + "figure on it: how much, from what to what, over how long.";
      }
      return "Every bullet states a result. Nothing left to fix by measurement here.";
    },
    bullets: (f) => f.total === 0
...
const BIEN = {
  fr: {
    ...
    // La table BIEN sert quand l'axe est deja bon. Sans entree ici, phrase()
    // rendait undefined et le tableau affichait un vide : un axe reussi
    // n'avait rien a dire, ce qui se lit comme une panne.
    achievements: (f) => f.resultat + " puce(s) sur " + f.total
      + " disent un resultat, pas seulement une responsabilite.",
  ...
  en: {
    ...
    achievements: (f) => f.resultat + " of " + f.total
      + " bullets state a result, not just a responsibility.",
