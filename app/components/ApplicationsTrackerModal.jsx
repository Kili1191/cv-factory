...
function ApplicationCard({ T, app, onEdit, onDelete, onAction }) {
  ...
  return (
    <div style={{
      ...
      {/* THE CV THAT LEFT
          Kept on the row by the fit or the writer. One tap puts it back in
          the editor, to download it again or to refit it: with a hundred
          applications, "which CV did I send them" has to be one tap. */}
      {app.cv && (
        <button data-nuvi="ap-reopen" onClick={()=>onAction && onAction("reopen", app)} style={{
          ...B({
            width:"100%", minHeight:44, marginBottom:8,
