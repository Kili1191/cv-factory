...
const LayoutPreview = ApercuGabarit;
...
export default function App() {
  ...
  // LE ROUTEUR DE PHASES A ETE RETIRE
  //
  // Il exposait `phase` et `setPhase` pour piloter le couple (tab, aiMode)
  // depuis une barre en trois temps, Demarrer / Cibler / Finaliser. Cette
  // barre n'a jamais ete rendue : `setPhase` n'avait donc aucun appelant, et
  // `phase` etait calcule puis jete a chaque rendu. Toute l'abstraction
  // tenait debout sans que rien ne s'y appuie.
    ...
    <ContexteLireImage.Provider value={lireImageCv}>
      ...
      <div data-cvf="app" className="nuvi-bg-halos" style={{
        ...
        <div style={{flex:1, overflowY:"auto",
          ...
          position:"relative", zIndex:1}}>
          {/* Sur mobile, le contenu inline est l'AITabContent (Demarrer) par
              defaut. Les autres sections ouvrent des modales via
              NuviBottomNav, donc pas besoin d'affichage inline.

              FINALIZECONTENT A ETE RETIRE, ET LE COMMENTAIRE QUI LE GARDAIT
              AUSSI. Il disait qu'on le conservait "au cas ou un onglet
              legacy l'active". Aucun ne l'activait : `setTab` n'est appele
              qu'avec "ai" et "target", jamais avec "edit", "design", "score"
              ni "tools". Quatre cent quatre-vingts lignes attendaient un
              etat que rien ne pouvait produire, et le "au cas ou" tenait
              lieu de verification. */}
          {tab==="ai"     && AITabContent}
